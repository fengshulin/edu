...
public class RedisUtil {
	private static PropertiesLoader propertiesLoader = new PropertiesLoader("redis.properties");
	...
	private RedisUtil() {
		if (pool == null) {
			String ip = propertiesLoader.getProperty("redis.host");
			int port = propertiesLoader.getInteger("redis.port");
			JedisPoolConfig config = new JedisPoolConfig();
			// 控制一个pool可分配多少个jedis实例，通过pool.getResource()来获取；
			// 如果赋值为-1，则表示不限制；如果pool已经分配了maxActive个jedis实例，则此时pool的状态为exhausted(耗尽)。
			config.setMaxTotal(propertiesLoader.getInteger("redis.pool.maxTotal"));
			// 控制一个pool最多有多少个状态为idle(空闲的)的jedis实例。
			config.setMaxIdle(propertiesLoader.getInteger("redis.pool.maxIdle"));
			// 表示当borrow(引入)一个jedis实例时，最大的等待时间，如果超过等待时间，则直接抛出JedisConnectionException；
			config.setMaxWaitMillis(propertiesLoader.getInteger("redis.pool.maxWaitMillis"));
